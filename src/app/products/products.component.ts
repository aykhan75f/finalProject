...
export class ProductsComponent implements OnInit {
  ...
  applyRatingFilter(min: number) {
    if (this.selectedRatingFilters.includes(min)) {
      this.selectedRatingFilters = this.selectedRatingFilters.filter(
        (val) => val !== min
      );
    } else {
  ...
  applyPriceFilter(min: number, max: number) {
    ...
    if (this.isPriceFilterSelected(filter)) {
      this.selectedPriceFilters = this.selectedPriceFilters.filter(
        (val) => val.min !== filter.min || val.max !== filter.max
      );
    } else {
  ...
  filterProducts() {
    ...
    if (this.selectedRatingFilters.length > 0) {
      filteredProducts = filteredProducts.filter((product: any) => {
        return this.selectedRatingFilters.includes(
    ...
    if (
      this.selectedRatingFilters.length === 0 &&
  ...
  isPriceFilterSelected(filter: { min: number; max: number }): boolean {
    return this.selectedPriceFilters.some(
      (f) => f.min === filter.min && f.max === filter.max
    );
  }
}
